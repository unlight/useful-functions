...
function d() {
	...
	if (!response) {
		...
		if (!(response instanceof ServerResponse)) response = false;
	}
	for (var i = 0; i < args.length; i++) {
		var h = new Array(i + 2).join('*');
		var arg = inspect(args[i], true, null);
		console.log(h, arg);
	}
	...
	if (response) {
		...
		response.write("<pre>");
		response.write(inspect(args, true, null));
		response.write("</pre>");
		response.end();
	}
}
...
/*module.exports = function() {
	...
	self.fileGetContents = function(filePath) {
		if (fs.existsSync(filePath)) {
	...
	self.getClassName = function(object) {
		var result;
...
// }
	...
	self.insideCut = function(string, length, separator) {
		if (!self.isString(string)) {
			string = string.toString();
		}
		if (string.length > length) {
	...
	self.exceptionHandler = function(error) {
		var messageInfo = error.message.split(' ');
		var message = error.message;
		var senderObject;
		var senderMethod;
		...
		if (messageInfo.length == 4) {
			...
			message = self.insideCut(message, 80);
		}
		var stack = error.stack;
		var stackArray = stack.split("\n    at ");
		var filePath;
		...
		var os = require('os');
		var serverResponse = require('garden').serverResponse;
		...
		if (isArray(errorLines) && lineNumber > -1) {
			...
			for (var i = 0, length = errorCodeLines.length, last = length - 1; i < length; i++) {
				...
				padding = padding + '   ';
				padding = padding.slice(0, 3) + ' ' + fileLineNumber + ':';
				var line = (padding + ' ' + self.escapeHtml(errorCodeLines[i])).trim();
				if (i != last) {
					line += "\n";
				}
				errorCodeTrace += '<span' + (cssClass ? ' class="'+cssClass+'"' : '') + '>' + line + '</span>';
		...
		if (panicError == true) {
			if (deliveryType == 'ALL') {
				var deverror = garden.path.root + '/applications/garden/design/deverror.css';
				var styleCss = self.fileGetContents(deverror);
				if (styleCss) {
					styleCss = ['<style type="text/css">', styleCss, '</style>'].join("\n");
				}
				var html = [
					...
					'<h1>' + 'Fatal Error in ' + senderObject + '.' + senderMethod + '()' + '</h1>',
					'<div id="Content">',
					'<h2>' + self.escapeHtml(message) + '</h2>',
					(senderCode) ? '<code>' + self.escapeHtml(senderCode) + '</code>' : '',
					(filePath) ? '<h3>The error occurred on or near: <strong> ' + filePath + '</strong></h3>' : '',
					(errorCodeTrace) ? '<div class="PreContainer">' + errorCodeTrace + '</div>' : '',
					'<h3><strong>Backtrace:</strong></h3>',
					// TODO: escapeHtml on backtrace
					...
					'<h3><strong>Additional information:</strong></h3>',
					'<ul>',
					'<li><strong>Error:</strong> ' + self.escapeHtml(message) + '</li>',
					// '<li><strong>Application:</strong> ' + '' + '</li>',
					// '<li><strong>Application Version:</strong> ' + '' + '</li>',
					'<li><strong>Operating System:</strong> ' + [os.platform(), os.release()].join(' ') + '</li>',
					'<li><strong>Server Software:</strong> ' + ['node', versions.node, 'v8', versions.v8].join(' ') + '</li>',
	...
	self.d = function() {
		...
		for (var i = 0; i < arguments.length; i++) {
			dumpArray[dumpArray.length] = arguments[i];
			var h = new Array(i + 2).join('*');
			console.log(h, arguments[i]);
		}
		var serverResponse = require('garden').serverResponse;
		if (serverResponse) {
			serverResponse.setHeader('Content-Type', 'text/html; charset=utf-8');
			serverResponse.write(inspect(dumpArray));
			serverResponse.end();
		}
	}
